...
def eval(
    ...
    with torch.no_grad():
        for images, global_labels in progress_bar:
            ...
            # Ensure images are cast to the dtype of the visual model's first conv layer's weights
            target_dtype = clip_model_visual.conv1.weight.dtype
            ...
            # MetaNet usually expects float32 for its inputs.
            ...
            # Ensure delta_ctx is cast to cocoop_model.ctx.dtype before addition
            ...
            transformer_dtype = next(cocoop_model.transformer.parameters()).dtype
            ...
            x = cocoop_model.transformer(x)  # Already in correct dtype
